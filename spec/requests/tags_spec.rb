  ...
  context "Tag collection" do
    ...
    context "with an appropriate role" do
      it "can create a tag with category by href" do
        ...
        expect { post api_tags_url, options }.to change(Tag, :count).by(1)
      ...
      it "can create a tag with a category by id" do
        ...
        expect do
          post api_tags_url, :name => "test_tag", :description => "Test Tag", :category => {:id => category.id}
      ...
      it "can create a tag with a category by name" do
        ...
        expect do
          post api_tags_url, :name => "test_tag", :description => "Test Tag", :category => {:name => category.name}
        end.to change(Tag, :count).by(1)
      ...
      it "can create a tag as a subresource of a category" do
        ...
        expect do
          post(api_category_tags_url(nil, category), :name => "test_tag", :description => "Test Tag")
        end.to change(Tag, :count).by(1)
        tag = Tag.find(ApplicationRecord.uncompress_id(response.parsed_body["results"].first["id"]))
        tag_category = Category.find(tag.category.id)
        expect(tag_category).to eq(category)
      ...
      it "returns bad request when the category doesn't exist" do
        ...
        post api_tags_url, :name => "test_tag", :description => "Test Tag"
      ...
      it "can update a tag's name" do
        ...
        expect do
          post api_tag_url(nil, tag), gen_request(:edit, :name => "new_name")
      ...
      it "can update a tag's description" do
        ...
        expect do
          post api_tag_url(nil, tag), gen_request(:edit, :description => "New Description")
      ...
      it "can delete a tag through POST" do
        ...
        expect { post api_tag_url(nil, tag), :action => :delete }.to change(Tag, :count).by(-1)
      ...
      it "will respond with 404 not found when deleting a non-existent tag through POST" do
        ...
        post api_tag_url(nil, tag_id), :action => :delete
      ...
      it "can delete multiple tags within a category by id" do
        ...
        expect do
          post(api_category_tags_url(nil, category.id), gen_request(:delete, [{:id => tag1.id}, {:id => tag2.id}]))
      ...
      it "can delete multiple tags within a category by name" do
        ...
        expect do
          post(api_category_tags_url(nil, category), body)
        end.to change(Tag, :count).by(-2)
        expect { classification1.reload }.to raise_error(ActiveRecord::RecordNotFound)
        expect { classification2.reload }.to raise_error(ActiveRecord::RecordNotFound)
        expect_result_to_match_hash(
    ...
    context "without an appropriate role" do
      ...
      it "cannot update a tag" do
        ...
        expect do
          post api_tag_url(nil, tag), gen_request(:edit, :name => "New name")
      ...
      it "cannot delete a tag through POST" do
        ...
        expect { post api_tag_url(nil, tag), :action => :delete }.not_to change(Tag, :count)
    ...
    it "query tags with expanded resources" do
      ...
      get api_tags_url, :expand => "resources"
    ...
    it "query tag details with multiple virtual attributes" do
      ...
      get api_tag_url(nil, tag), :attributes => attr_list
    ...
    it "query tag details with categorization" do
      ...
      tag = Tag.last
      get api_tag_url(nil, tag), :attributes => "categorization"
    ...
    it "query all tags with categorization" do
      ...
      get api_tags_url, :expand => "resources", :attributes => "categorization"
