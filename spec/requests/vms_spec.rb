...
describe "Vms API" do
  ...
  context "Vm set_description action" do
    ...
    it "to a single vm" do
      ...
      expect_single_action_result(:success => true, :message => /#{vm.id}.* setting description to test/i, :href => api_vm_url(nil, vm))
    end
    ...
    it "to multiple vms" do
      ...
      expected = {
        "results" => a_collection_containing_exactly(
          a_hash_including(
            "message" => a_string_matching(/#{vm1.id}.* setting description to test1/i),
            "success" => true,
            "href"    => api_vm_url(nil, vm1)
          ),
          a_hash_including(
            "message" => a_string_matching(/#{vm2.id}.* setting description to test2/i),
            "success" => true,
            "href"    => api_vm_url(nil, vm2)
          )
        )
