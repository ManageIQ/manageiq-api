RSpec.shared_context "SubResource#cancel" do |ns, request_factory, factory|
  ...
  let(:request)        { FactoryGirl.build(request_factory).tap { |i| i.try(:requester=, @user); i.user = @user if i.kind_of?(ServiceOrder); i.save } }
  let(:resource_1)     { FactoryGirl.build(factory).tap { |i| i.try(:requester=, @user); i.save } }
  let(:resource_2)     { FactoryGirl.build(factory).tap { |i| i.try(:requester=, @user); i.save } }
